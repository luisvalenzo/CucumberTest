...
public class ManagementRolesByPerson {
...
//		given().when().get("http://www.google.com").then().statusCode(200);
		...
		List<HashMap<String, Object>> jsonList = jsonResponse.getList("managementStructure");
	    //System.out.println(jsonList.get(0).get("auditRollupId"));	
		int listSize = jsonList.size();
		System.out.println("List size " + listSize );
		String[] id = new String[listSize];
		for (int i = 0; i < listSize; i++) {
			id[i] = (String) jsonList.get(i).get("entityName");
